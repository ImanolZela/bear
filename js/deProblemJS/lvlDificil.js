document.addEventListener('DOMContentLoaded', function() {
    ...
    let bearBotPosition = {row: 6, col: 0}; // Posición inicial en la parte izquierda inferior
    let remainingMoves = 0;
    let diceRolling = false;
    const rows = 7;
    const cols = 8;
    const images = ['../media/OBS 1.svg', '../media/OBS 2.svg'];
    const startEndPositions = [[6, 0, '../media/OSO CABEZA.svg', 'INICIO'], [0, 7, '../media/NAVE+BEARBOT.svg', 'FIN']]; // Ajusta la posición de inicio y fin
    ...
    function placeRandomAliens() {
        ...
        for (let i = 0; i < rows; i++) {
            for (let j = 0; j < cols; j++) {
                if ((i !== bearBotPosition.row || j !== bearBotPosition.col) && !(i === 0 && j === 7)) {
        ...
        const pathToFinish = [
            ...
            {row: 5, col: 7}, {row: 4, col: 7}, {row: 3, col: 7}, {row: 2, col: 7}, {row: 1, col: 7}, {row: 0, col: 7}
        ];
        ...
        const numberOfAliens = 15; // 9 aliens
